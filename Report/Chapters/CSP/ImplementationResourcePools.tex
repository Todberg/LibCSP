...
The resource pools are managed in an \code{ResourcePool} object. This uses the queue data structure to store unused \code{SocketCore}, \code{ConnectionCore} and \code{PacketCore} objects. A separate queue is created for each of these. During the \textit{initialization phase} the \code{CSPManager} will initialise the pools with fixed amount of objects. As illustrated in Listing \ref{poolinit}, this is done simply by enqueuing new instantiations of the objects. All are therefore allocated in mission memory and none of these will end up referencing to other objects in a shorter lived scope. 
\lstinputlisting[label=poolinit,caption=Initialising the pool for packets.]
